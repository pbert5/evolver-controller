...
OWNED_UNITS = ("evolver-controller.service",)
OWNED_LINKS = ("evoctl", "evolver-controller")
...
def repair_installation(state_root: str | Path, *, native_root: str | Path = DEFAULT_NATIVE_ROOT,
    ...
    if not all(item.is_file() and os.access(item, os.X_OK) for item in (controller, ctl)):
        raise RuntimeError("current release is incomplete; reinstall a verified release")
    links = {"evoctl": ctl, "evolver-controller": controller}
    ...
    (systemd / OWNED_UNITS[0]).write_text(systemd_unit(executable=str(controller), state_root=str(root), install_config=not control), encoding="utf-8")
    if control:
...
EOF
  ...
  [ -x "$RELEASE_STAGE/bin/evoctl" ] && [ -x "$RELEASE_STAGE/bin/evolver-controller" ] || {{ echo "fresh release is incomplete" >&2; exit 65; }}
...
install_software() {{
  ...
  install_native
  [ -x "$TARGET_CTL" ] && [ -x "$TARGET_CONTROLLER" ] || {{ echo "controller installation did not provide required executables" >&2; exit 69; }}
}}
case "$MODE" in
...
if [ "$MODE" = update ] || [ "$MODE" = clean-reinstall ] || [ "$MODE" = install ]; then
  EVOCTL="$NATIVE_ROOT/current/bin/evoctl"; CONTROLLER="$NATIVE_ROOT/current/bin/evolver-controller"
else
  EVOCTL="$CURRENT_CTL"; CONTROLLER="$NATIVE_ROOT/current/bin/evolver-controller"
fi
...
if systemctl is-active --quiet evolver-controller.service; then
  ...
  systemctl restart evolver-controller.service
else
  if [ "$SYSTEMD_NIXOS_CONTROL" = true ]; then
    ...
    systemctl enable --runtime --now evolver-controller.service
  else
    systemctl enable --now evolver-controller.service
  fi
fi
if ! systemctl is-active --quiet evolver-controller.service; then
...
EOF
  ...
  systemctl is-active --quiet evolver-controller.service || {{ echo "controller update health check failed" >&2; exit 70; }}
fi
