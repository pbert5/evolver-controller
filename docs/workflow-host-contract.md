...
- physical `set_temperature` is available only when the target projects
  hardware protocol v2 and the exact immutable per-vial temperature
  calibration preflight is eligible; v1 devices and missing, stale,
  ambiguous, or out-of-range calibration remain unavailable before side
  effects. Simulator targets report `simulator_only`;
